...
namespace MadGoat_SSAA
{
    ...
    public class MadGoatSSAA_InternalRenderer : MonoBehaviour
    {
        ...
        private void UpdateCBSRP()
        {
#if UNITY_2018_1_OR_NEWER && UNITY_POST_PROCESSING_STACK_V2 && SSAA_HDRP
            // setup shader pass
            ((SsaaSamplingUber)PostVolume.sharedProfile.settings[0]).shaderPass.value = PostVolumePass;
            ((SsaaSamplingUber)PostVolume.sharedProfile.settings[0]).sourceTex.value = Main.targetTexture;
            ((SsaaSamplingUber)PostVolume.sharedProfile.settings[0]).useFXAA.value = 
            ...
            // setup shader specs
            ((SsaaSamplingUber)PostVolume.sharedProfile.settings[0]).resizeWidth.value = Screen.width;
            ((SsaaSamplingUber)PostVolume.sharedProfile.settings[0]).resizeHeight.value = Screen.height;
            ((SsaaSamplingUber)PostVolume.sharedProfile.settings[0]).sharpness.value = Sharpness;
            ((SsaaSamplingUber)PostVolume.sharedProfile.settings[0]).sampleDistance.value = SampleDistance;
            ((SsaaSamplingUber)PostVolume.sharedProfile.settings[0]).flip.value = MainComponent.flipImageFix && GraphicsSettings.renderPipelineAsset.ToString().Contains("(UnityEngine.Experimental.Rendering.HDPipeline.HDRenderPipelineAsset)");
#endif
        }
